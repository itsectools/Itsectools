...
export const metadata: Metadata = {
    title: 'Test Your DLP Policy — Nested JSON Exfiltration & Advanced DLP Testing | ITSecTools',
    description: 'The only free DLP testing tool with Advanced DLP Tests: Nested JSON Exfiltration (MCP/API payloads), multi-protocol uploads (HTTP/HTTPS/FTP), endpoint DLP detection, PDF validation reports, and vendor-specific regex for 10 engines. No signup.',
    alternates: {
        canonical: 'https://itsectools.com/dlp',
    },
    openGraph: {
        title: 'Test Your DLP Policy — Nested JSON Exfiltration & Advanced DLP Testing | ITSecTools',
        description: 'The only free tool that tests nested JSON exfiltration — can your DLP detect sensitive data buried inside MCP/API payloads? Plus multi-protocol uploads, endpoint DLP detection, and PDF reports.',
...
const jsonLd = {
    ...
    featureList: [
        ...
        'Nested JSON Exfiltration (MCP/API Payloads) — test whether DLP detects sensitive data inside deeply nested JSON structures used by AI agents, REST APIs, and GraphQL mutations'
    ]
};
...
const faqJsonLd = {
    ...
    mainEntity: [
        ...
        {
            '@type': 'Question',
            name: 'What is Nested JSON Exfiltration testing and why does it matter for DLP?',
            acceptedAnswer: { '@type': 'Answer', text: 'Nested JSON Exfiltration tests whether your DLP can detect sensitive data (PII, PCI, PHI) buried inside deeply nested JSON structures at configurable depths (2, 4, or 6 levels). AI agents (MCP), REST APIs, and GraphQL mutations all transmit data in nested JSON. Many DLP engines only scan flat text — this test reveals whether they can parse structured payloads to find sensitive data hidden multiple levels deep.' }
        },
        {
            '@type': 'Question',
