...
export default function DLPClient() {
    ...
    const handleMcpTest = async () => {
        ...
        try {
            ...
            if (!genRes.ok) { setMcpStatus('ERROR: Failed to generate nested JSON payload.'); return; }
            ...
            if (testRes.ok) {
                setMcpStatus(`✅ DATA LEAKED: ${mcpProtocol} nested JSON POST successful — DLP failed to detect ${mcpDataType.toUpperCase()} data at depth ${mcpDepth}.`);
                addTestResult('mcp', mcpProtocol, 'leaked', `JSON ${mcpDataType.toUpperCase()} (depth ${mcpDepth})`);
            } else {
                setMcpStatus(`🚫 BLOCKED: ${mcpProtocol} nested JSON POST rejected — HTTP ${testRes.status}. DLP detected nested ${mcpDataType.toUpperCase()} data.`);
                addTestResult('mcp', mcpProtocol, 'blocked', `JSON ${mcpDataType.toUpperCase()} (depth ${mcpDepth})`);
            }
        } catch (err: any) {
            if (err.name === 'AbortError') {
                setMcpStatus(`🚫 BLOCKED: ${mcpProtocol} nested JSON POST timed out (dropped by DLP Agent).`);
            } else {
                setMcpStatus(`🚫 BLOCKED: ${mcpProtocol} nested JSON POST failed — intercepted by DLP Agent.`);
            }
            addTestResult('mcp', mcpProtocol, 'blocked', `JSON ${mcpDataType.toUpperCase()} (depth ${mcpDepth})`);
        }
    };
                    ...
                    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                        ...
                        <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '2rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                                ...
                                <h4 style={{ margin: 0, fontSize: '1.1rem', color: '#0F172A' }}>Nested JSON Exfiltration (MCP/API Payloads)</h4>
                            </div>
                            <p style={{ fontSize: '0.85rem', color: '#64748B', marginBottom: '1.5rem', marginTop: '0.25rem' }}>
                                Tests whether your DLP can parse deeply nested JSON structures used by AI agents (MCP), REST APIs, and GraphQL mutations to detect sensitive data.
                                Data is generated server-side — the DLP engine must inspect the network traffic to find it.
                            </p>
                            ...
                            <div style={{ maxWidth: '600px' }}>
                                ...
                                <button className="btn-outline" onClick={handleMcpTest}>
                                    <PlayIcon width={16} height={16} /> Send Nested JSON Test
                                </button>
                            </div>
