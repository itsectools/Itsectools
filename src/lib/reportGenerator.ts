...
export async function generateDLPReport(
    ...
    if (leakedProtocols.includes('MCP')) {
        gaps.push('Nested JSON exfiltration not detected — DLP cannot parse deeply nested JSON payloads (MCP/API format)');
        recs.push('Enable deep content inspection for JSON payloads and configure DLP to parse nested structures used by AI agents (MCP), REST APIs, and GraphQL mutations');
    }
